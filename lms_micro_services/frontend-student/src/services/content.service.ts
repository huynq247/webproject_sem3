...
export const contentService = {
  ...
  // Study Progress
  saveStudyProgress: async (progressData: {
    deck_id: string;
    ...
    console.log('💾 Saving study progress:', progressData);
    try {
      const response = await apiClient.post('/api/study-progress/', progressData);
      console.log('📊 Study progress saved:', response.data);
      return response.data;
    } catch (error) {
  ...
  getStudyProgress: async (deckId: string) => {
    console.log('📊 Getting study progress for deck:', deckId);
    try {
      const response = await apiClient.get(`/api/study-progress/deck/${deckId}`);
      console.log('📈 Study progress response:', response.data);
      return response.data;
    } catch (error) {
  ...
  getUserStudyStats: async () => {
    console.log('📊 Getting user study stats');
    try {
      const response = await apiClient.get('/api/study-progress/stats');
      console.log('📈 Study stats response:', response.data);
      return response.data;
    } catch (error) {
      console.error('❌ Study stats get error:', error);
      throw error;
    }
  },
};
